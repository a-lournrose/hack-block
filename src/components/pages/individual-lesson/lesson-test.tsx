...
export const LessonTest:FC<ILessonTest> = ({questions}) => {
  ...
  const [questionsState, setQuestionsState] = useState<QuestionDto[]>(questions);

  useEffect(() => {
    setQuestionsState(prev => prev.map(question => {
      return {
        ...question,
  ...
  const contents = questionsState.map((question, index) => <TestItemCard question={question}
                                                                    onCheckedChange={onCheckedChange}
  />);
  return (
    <LessonContainer>
      <div className="p-8">
        <FlippingContainer contents={contents} onFinish={onFinish}/>
      </div>
    </LessonContainer>
  );
};
